...
Such a starts with an uppercase letter, which may be followed by by an arbitrary number of letters, digits, apostrophes and underscores.
...
A sequence of names, separated by dots. The last part must be a construtor name.

### Native type names

A fully qualified name of a class or an interface. If it contains characters  that are not allowed in Frege (like `$`) or words that are keywords, it can be given as a string literal.
